...
const state = {
  ...
  device: {
    connected: false,
    touching: false,
    pressTimes: [], // for 2x / 3x detection
  }
};
...
// --- Device Simulator Logic (lights & multi-press) ---
...
function bindSimulator(root){
  ...
  if (btnConnect) btnConnect.onclick = ()=>{
    // Simulate connect flow: yellow -> connected
  ...
  if (btnTouch) btnTouch.onclick = ()=>{
    ...
    updateLights(root);
    // if touching -> auto open app section (stay on current screen in demo)
  };
...
function bindPractice(root){
  ...
  function tick(){
    ...
    if (pracRemaining<=0){
      ...
      new AudioContext().resume?.().catch(()=>{}); // best effort un-suspend
      return;
    }
  }
  ...
  $('#prac-stop', root).onclick = ()=>{ if (pracInterval){ clearInterval(pracInterval); pracInterval=null; } };

  $('#btn-correct', root).onclick = ()=>{
    correct++; $('#correct-count', root).textContent = correct;
    if (correct===10){
...
function bindGame(root){
  ...
  const g1Start = $('#g1-start', root);
  $('#g1-ready', root).onclick = ()=>{ g1Ready=true; g1Start.disabled=false; g1F.textContent='Ready!'; };
  $('#g1-start', root).onclick = ()=>{
    ...
    g1Interval = setInterval(()=>{
      ...
      if (g1Remaining<=0){ clearInterval(g1Interval); g1Interval=null; g1F.textContent = feedbackForPoints(g1Points, +$('#g1-secs', root).value||60); }
    },1000);
  };
  ...
  $('#g1-untouch', root).onclick = ()=>{ /* no-op in demo */ };

  function feedbackForPoints(points, secs){
    // Normalize to 60s thresholds as in spec
  ...
  $('#g2-start', root).onclick = ()=>{
    ...
    g2Interval = setInterval(()=>{
      ...
      if (g2Remaining<=0){ clearInterval(g2Interval); g2Interval=null; g2Feedback.textContent = feedbackForPoints(g2Score, +$('#g2-secs', root).value||60); }
    },1000);
  };
